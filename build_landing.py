...
<link rel="icon" type="image/png" href="logo_shield.png">
<link rel="apple-touch-icon" href="logo_shield.png">
...
function sendForm(e){
  e.preventDefault();
  const f=e.target;
  const name=f.name.value, phone=f.phone.value, email=f.email.value, msg=f.message.value;
  const body=`שם: ${name}\nטלפון: ${phone}\nמייל: ${email}\n\nהודעה:\n${msg}`;
  window.location.href='mailto:info@shk.org.il?subject=פנייה מהאתר — '+encodeURIComponent(name)+'&body='+encodeURIComponent(body);
  f.style.display='none';
  document.getElementById('cfThanks').style.display='block';
}
</script>
</body>
</html>"""
