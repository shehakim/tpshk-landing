...
PRODUCTS = [
    ...
    {"id":"mapping",   "name":"בדיקת מיפוי חינוך מיוחד",                     "rgb":"110,146,92",  "live":False, "url":"mh.tpshk.org.il",
    ...
    {"id":"nitzanim", "name":"בקרת תכנית ניצנים",                          "rgb":"108,118,180", "live":False, "url":"nz.tpshk.org.il",
     "desc":'בקרה תקציבית ותפעולית מלאה על תכנית "ניצנים" ברשות המקומית. המערכת נמשכת אוטומטית את נתוני התקצוב העדכניים ממית"ר ומצליבה אותם מול דיווחי הביצוע החודשיים, מאתרת פערים וטעויות דיווח, ומפיקה דוחות חלוקה למפעילים ולמוסדות לצד צפי תקציב שנתי — שליטה מלאה בכל שקל של התכנית.'},
]
for _p in PRODUCTS:
    _p["icon"] = _icon(_p["id"])

# מיקומי השמש [x%, y%] — 11 נקודות על אליפסה סביב הלוגו, מרווחים שווים (כל 32.7°)
NODE_POS = [
    [50, 19],  # ניהול כח אדם (ארוך) — מרכז, עליון
    [66, 24],  # בניית תקציב         — ימין, עליון
    [77, 37],  # ניהול תיכונים       — ימין, עליון-אמצע
    [80, 54],  # הסעות (ארוך)        — ימין, אמצע
    [73, 70],  # פקודת יומן          — ימין, תחתון-אמצע
    [58, 80],  # אגרות חוץ           — מרכז-ימין, תחתון
    [42, 80],  # בקרת כח אדם         — מרכז-שמאל, תחתון
    [27, 70],  # שכר תיכונים (ארוך)  — שמאל, תחתון-אמצע
    [20, 54],  # אבטחת מוסדות חינוך  — שמאל, אמצע
    [23, 37],  # מיפוי חנ״מ          — שמאל, עליון-אמצע
    [34, 24],  # בקרת תכנית ניצנים   — שמאל, עליון
]

# מיקומי טבעת ייעודיים לפלאפון — אובל אנכי סימטרי סביב הלוגו (11 נקודות כל 32.7°)
MOBILE_NODE_POS = [
    [50, 12],  # למעלה
    [69, 18],  # עליון-ימין
    [82, 34],  # ימין עליון
...
<div class="above-fold">
  ...
  <div class="hub-section" id="hubSection">
    ...
    <!-- 11 product nodes — icon + name only, placed by JS -->
__NODES__
  </div><!-- /hub-section -->
</div><!-- /above-fold -->
...
<script>
// 11 product nodes — balanced ring [xPct, yPct], kept inside bounds
const POS = __POS__;
