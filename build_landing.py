...
# מיקומי השמש [x%, y%] — 10 נקודות על אליפסה סביב הלוגו, מרווחים שווים (כל 36°)
NODE_POS = [
    [50, 19],  # ניהול כח אדם (ארוך) — מרכז, עליון
    [68, 25],  # בניית תקציב         — ימין, עליון
    [78, 40],  # ניהול תיכונים       — ימין, עליון-אמצע
    [78, 60],  # הסעות (ארוך)        — ימין, תחתון-אמצע
    [68, 75],  # פקודת יומן          — ימין, תחתון
    [50, 81],  # אגרות חוץ           — מרכז, תחתון
    [32, 75],  # בקרת כח אדם         — שמאל, תחתון
    [22, 60],  # שכר תיכונים (ארוך)  — שמאל, תחתון-אמצע
    [22, 40],  # אבטחת מוסדות חינוך  — שמאל, עליון-אמצע
    [32, 25],  # מיפוי חנ״מ          — שמאל, עליון
]

# מיקומי טבעת ייעודיים לפלאפון — אובל אנכי סימטרי סביב הלוגו (10 נקודות כל 36°)
MOBILE_NODE_POS = [
    [50, 12],  # למעלה
    [71, 19],  # עליון-ימין
    [83, 38],  # ימין עליון
...
<div class="above-fold">
  ...
  <div class="hub-section" id="hubSection">
    ...
    <!-- 10 product nodes — icon + name only, placed by JS -->
__NODES__
  </div><!-- /hub-section -->
</div><!-- /above-fold -->
...
<script>
// 10 product nodes — balanced ring [xPct, yPct], kept inside bounds
const POS = __POS__;
